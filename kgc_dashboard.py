"""
KGC 인삼공사 · 주간 마케팅 통찰 보고서 (Streamlit 버전)
실행: streamlit run kgc_dashboard.py
"""
...
.brand-tag {
    ...
    color: #9a6f20; margin-bottom: 8px;
}
...
kpi_data = [
    (k1, "kpi-up",   "📈", "수도권 판매량 증감 (전주比)", "color-up",   "+15%", "편의점 채널 강세<br>MZ 접근성 확대 효과"),
    (k2, "kpi-down", "📉", "지방 판매량 증감 (전주比)",   "color-down", "−2%",  "대형마트 채널 정체<br>채널 다변화 검토 필요"),
    (k3, "kpi-gold", "🎯", "주력 구매 연령층",             "color-gold", "45%",  "2030 사회초년생<br>전체 구매자 중 비중"),
    (k4, "kpi-blue", "🏃", "액티브 키워드 언급 증가",      "color-blue", "+30%", "'등산·테니스' 함께 언급<br>신규 라이프스타일 연결 신호"),
]
for col, cls, badge, label, vcls, value, sub in kpi_data:
    with col:
        st.markdown(f"""
...
with ch_col:
    ...
    fig_bar.add_trace(go.Bar(
        x=[72, 55, 38, 28],
        y=["편의점 (수도권)", "온라인 (추정)", "대형마트 (지방)", "약국·헬스숍"],
        orientation="h",
        marker=dict(color=["#1e7a50","#2563c8","#c0392b","#c47a20"], opacity=0.85, line=dict(width=0)),
        text=["강세 ▲","성장 →","정체 ▼","기회 ◆"],
        textposition="outside",
        textfont=dict(size=11, color=["#1e7a50","#2563c8","#c0392b","#c47a20"]),
...
with age_col:
    ...
    fig_pie = go.Figure(go.Pie(
        labels=["2030 사회초년생", "4050 직장인", "60대 이상", "기타"],
        values=[45, 32, 14, 9],
        hole=0.52,
    ...
    st.markdown("""
    <p style="font-size:12px;color:#4a4a5a;line-height:1.7;font-weight:300;margin-top:-8px;">
      2030 비중이 기존 대비 <span style="color:#b8860b;font-weight:500">유의미하게 높아짐</span>.
      리뉴얼 패키지의 MZ 친화적 디자인이 신규 구매층 유입에 효과적으로 작동한 것으로 판단.
    </p>
    """, unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
with pos_col:
    ...
    <div class="panel">
      ...
      <div class="review-card review-pos">
        ...
        <div>
          <span class="review-tag tag-pos">패키지</span>
          <div class="review-text">"포장이 세련되어 선물용으로 좋다" — 리뉴얼 패키지가 선물 수요를 직접 자극. 기념일·명절 시즌 선물 기획전 연동 가능.</div>
        </div>
      </div>
      <div class="review-card review-pos">
        <div style="font-size:18px;">😋</div>
        <div>
          <span class="review-tag tag-pos">맛/편의성</span>
          <div class="review-text">"기존보다 쓴맛이 덜해 먹기 편하다" — 홍삼 복용 장벽 해소. 거부감 보유 소비자 재유입 포인트로 메시지화 필요.</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
with neg_col:
    ...
    <div class="panel">
      ...
      <div class="review-card review-neg">
        <div style="font-size:18px;">💰</div>
        <div>
          <span class="review-tag tag-neg">가격 민감도</span>
          <div class="review-text">"가격이 작년보다 오른 느낌이다" — 원가 인상 체감이 구매 이탈로 이어질 수 있음. 용량 대비 가치 소구 또는 구독형 구매 혜택 강조 필요.</div>
        </div>
      </div>
      <div class="review-card review-neg">
        <div style="font-size:18px;">📦</div>
        <div>
          <span class="review-tag tag-neg">패키지 불편</span>
          <div class="review-text">"박스 개봉이 가끔 뻑뻑하다" — 반복 구매 UX에 직접적 마찰. 품질팀과 협업하여 개봉 구조 개선 또는 박스 재질 검토 권고.</div>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    insights = [
        ("🏙️", "수도권·편의점 채널의 폭발적 성장",
         "수도권 +15%는 MZ층의 편의점 채널 구매 행동 변화가 반영된 신호. 편의점 진열 위치 및 POS 마케팅 집중 투자로 모멘텀 유지 필요.",
         "최우선", "p-high"),
        ("🌿", "지방 대형마트 채널 정체 대응",
         "지방 -2%는 대형마트 의존도 과다의 결과. 지방 편의점망 확대 및 지역 헬스숍·약국 채널로의 분산을 통해 유통 리스크 완화가 시급.",
         "중요", "p-mid"),
        ("🎾", "액티브 라이프 포지셔닝 전환 시점",
         "등산·테니스 키워드 +30% 급증은 소비자가 스스로 브랜드를 재정의하고 있다는 신호. 이 흐름을 마케팅이 따라가거나 선도해야 할 결정적 시점.",
         "최우선", "p-high"),
        ("💸", "가격 저항 선제 대응",
         "가격 민감도 언급은 향후 대량 이탈 예고 신호일 수 있음. '가성비 소구'보다 '가치 소구' 전환 — 구독 혜택·멤버십 할인 등 락인 장치 필요.",
         "중요", "p-mid"),
        ("📦", "패키지 완성도 보강",
         "선물용 호평과 박스 개봉 불편이 공존. 겉 디자인은 성공했으나 기능적 UX는 미완성. 품질팀·패키지 디자인팀 공동 개선 TF 구성 권고.",
         "검토", "p-low"),
        ("🍵", "맛 개선 자산화",
         '"쓴맛 감소"는 잠재 고객층을 실질적으로 넓힐 수 있는 산물. 광고 크리에이티브에서 전면 메시지화하여 홍삼 비경험자 유입 캠페인에 활용.',
         "검토", "p-low"),
    ]
...
with tab3:
    ...
    <table class="ch-table">
      <thead>
        <tr><th>채널</th><th>현황</th><th>기회 포인트</th><th>단기 액션 (4주 내)</th><th>우선순위</th></tr>
      </thead>
      <tbody>
        <tr>
          <td><span class="ch-badge" style="background:rgba(30,122,80,0.09);color:#1e7a50;border:1px solid rgba(30,122,80,0.3)">편의점</span></td>
          <td>수도권 +15% <span style="color:#1e7a50">강세</span></td>
          <td>2030 출근 전 루틴 수요, 충동 구매 유발 가능성</td>
          <td>계산대 옆 특화 진열 요청 · 스틱 번들 POP 소재 제작</td>
          <td><span class="ch-badge" style="background:rgba(192,57,43,0.09);color:#c0392b">최우선</span></td>
        </tr>
        <tr>
          <td><span class="ch-badge" style="background:rgba(37,99,200,0.08);color:#2563c8;border:1px solid rgba(37,99,200,0.25)">온라인</span></td>
          <td>성장세 유지 추정</td>
          <td>정기구독 전환율 제고, 리뷰 마케팅 연동</td>
          <td>네이버 스마트스토어 정기구독 패키지 출시 검토</td>
          <td><span class="ch-badge" style="background:rgba(196,122,32,0.09);color:#c47a20">우선</span></td>
        </tr>
        <tr>
          <td><span class="ch-badge" style="background:rgba(192,57,43,0.08);color:#c0392b;border:1px solid rgba(192,57,43,0.25)">대형마트</span></td>
          <td>지방 -2% <span style="color:#c0392b">정체</span></td>
          <td>선물세트 기획전 연동, 대용량 번들 프로모션</td>
          <td>점포별 담당MD와 4월 가정의 달 기획전 협의 착수</td>
          <td><span class="ch-badge" style="background:rgba(196,122,32,0.09);color:#c47a20">우선</span></td>
        </tr>
        <tr>
          <td><span class="ch-badge" style="background:rgba(184,134,11,0.09);color:#9a6f20;border:1px solid rgba(184,134,11,0.3)">약국·헬스숍</span></td>
          <td>미개발 채널</td>
          <td>액티브 라이프스타일 소비자 접점, 전문가 신뢰도</td>
          <td>약사·트레이너 대상 샘플 제공 파일럿 기획</td>
          <td><span class="ch-badge" style="background:rgba(30,122,80,0.08);color:#1e7a50">검토</span></td>
        </tr>
        <tr>
          <td><span class="ch-badge" style="background:rgba(0,0,0,0.05);color:#4a4a5a;border:1px solid rgba(0,0,0,0.07)">스포츠 채널</span></td>
          <td>신규 기회 감지</td>
          <td>등산·테니스 동호회 협찬, 스포츠 앱 제휴</td>
          <td>커뮤니티 매체(산악회 카페, 테니스 앱) 광고 단가 조사</td>
          <td><span class="ch-badge" style="background:rgba(30,122,80,0.08);color:#1e7a50">검토</span></td>
        </tr>
      </tbody>
    </table>
...
with tab4:
    actions = [
        ("01", "편의점 채널 판촉 소재 즉시 제작",
         '수도권 +15% 모멘텀을 유지하기 위해 2주 내 편의점용 POP 소재 및 계산대 옆 진열 협의 완료. "아침 루틴" 컨셉 메시지 탑재.',
         [("D-14","at-red"),("마케팅팀","at-blue"),("영업팀 협업","at-gray")]),
        ("02", "액티브 라이프 SNS 콘텐츠 캠페인 기획",
         '등산·테니스 키워드 +30% 급증 대응. 인스타그램·유튜브 쇼츠용 "운동 후 에브리타임" 브이로그형 콘텐츠 4월 중 런칭 목표.',
         [("D-21","at-red"),("디지털팀","at-blue"),("인플루언서 섭외","at-gray")]),
        ("03", "박스 개봉 UX 품질팀 이슈 공유",
         "리뷰 500건 중 반복 언급된 박스 개봉 불편 이슈를 품질팀·패키지 디자인팀에 공식 전달. 다음 생산 로트 전 개선 가능 여부 확인.",
         [("D-7","at-amber"),("품질팀 협업","at-blue"),("VOC 공식 이슈화","at-gray")]),
        ("04", "가정의 달 선물 기획전 대형마트 MD 협의",
         '"선물용으로 좋다" 긍정 리뷰를 활용. 5월 가정의 달 전 대형마트 선물세트 기획전 입점 협의를 이달 내 완료하여 지방 채널 반등 유도.',
         [("D-30","at-amber"),("영업팀","at-blue"),("대형마트 MD","at-gray")]),
        ("05", '"쓴맛 감소" 크리에이티브 소구 적용',
         '홍삼 비경험자 유입을 위한 "이제는 부담 없이" 컨셉 온라인 광고 소재 개발. 기존 건강기능 소구와 병행하여 A/B 테스트 진행.',
         [("D-45","at-green"),("광고팀","at-blue"),("A/B 테스트","at-gray")]),
    ]
  ...
  <div style="display:flex;gap:16px;align-items:flex-start;">
    ...
    <div>
      ...
      <p class="comment-text">
        ...
        이는 브랜드가 만든 포지셔닝이 아닌 소비자가 스스로 재정의한 이미지입니다.
